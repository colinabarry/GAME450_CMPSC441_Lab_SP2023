...
def get_route_cost(route_coordinate, game_map):
    ...
    grid: Grid = Grid(matrix=game_map)

    start: tuple[int, int] = grid.node(route_coordinate[0][0], route_coordinate[0][1])
    end: tuple[int, int] = grid.node(route_coordinate[1][0], route_coordinate[1][1])

    finder: AStarFinder = AStarFinder(diagonal_movement=DiagonalMovement.always)
